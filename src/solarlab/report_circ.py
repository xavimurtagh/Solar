"""Part III report: circularity and the urban mine.

Mirrors :mod:`solarlab.report_econ`. ``generate_circularity`` runs the dynamic
material-flow analysis and writes ``output/REPORT_CIRCULARITY.md`` plus figures
11-13, deterministically.
"""
...
from . import circularity as Z
from . import constants as C
from . import figures as figs
from . import materials as M
from .report import DEFAULT_OUTDIR, _md_table

HORIZON = 2080            # long view: shows the post-growth recycling catch-up
RECOVERY_PROCESSES = ["standard", "frelp", "hydro"]
...
class CircResults:
    ...
    fleet: pd.DataFrame
    silver_frelp: pd.DataFrame
    silver_standard: pd.DataFrame
    indium_frelp: pd.DataFrame
    recovery: pd.DataFrame
    crossover_year: object
    circ_2050_pct: float
    mass_2050_mt: float
    virgin_silver_avoided_t: float
    ceiling_2050: float
    ceiling_2080: float


def collect_circularity_results() -> CircResults:
    deployment = Z.project_deployment(to_year=HORIZON)
    fleet = Z.fleet_flows(to_year=HORIZON)

    silver_frelp = Z.material_flow("TOPCon", "Silver", process="frelp", to_year=HORIZON)
    silver_standard = Z.material_flow("TOPCon", "Silver", process="standard", to_year=HORIZON)
    indium_frelp = Z.material_flow("Perovskite-Si tandem", "Indium",
                                   process="frelp", to_year=HORIZON)

    recovery = Z.load_recovery()

    crossover = Z.circularity_crossover(silver_frelp, 0.5)
    circ_2050 = float(silver_frelp.loc[silver_frelp["year"] == 2050, "circularity"].iloc[0]) * 100
    mass_2050 = Z.cumulative_recovered_mass_mt(2050)
    virgin = Z.cumulative_virgin_avoided_t("TOPCon", "Silver", "frelp", to_year=HORIZON)
    c2050 = float(silver_frelp.loc[silver_frelp["year"] == 2050, "ceiling_circular_tw"].iloc[0])
    c2080 = float(silver_frelp.loc[silver_frelp["year"] == 2080, "ceiling_circular_tw"].iloc[0])

    return CircResults(
        deployment=deployment, fleet=fleet, silver_frelp=silver_frelp,
        silver_standard=silver_standard, indium_frelp=indium_frelp,
        recovery=recovery, crossover_year=crossover, circ_2050_pct=circ_2050,
        mass_2050_mt=mass_2050, virgin_silver_avoided_t=virgin,
        ceiling_2050=c2050, ceiling_2080=c2080)


def render_circularity_figures(r: CircResults, outdir: Path) -> list[Path]:
    figdir = outdir / "figures"
    ceiling_flows = {
        "Silicon (silver-limited)": r.silver_frelp,
        "Tandem (indium-limited)": r.indium_frelp,
    }
    return [
        figs.fig11_urban_mine(r.fleet, figdir),
        figs.fig12_relaxed_ceiling(ceiling_flows, M.NET_ZERO_TW_PER_YEAR,
                                   r.crossover_year, figdir),
        figs.fig13_circularity(r.silver_frelp, r.silver_standard,
                               r.virgin_silver_avoided_t, figdir),
    ]


def build_circularity_report(r: CircResults, outdir: Path = DEFAULT_OUTDIR) -> Path:
    ceil_linear = float(r.silver_frelp["ceiling_linear_tw"].iloc[0])

    recovery_tbl = _md_table(
        r.recovery[r.recovery["process"].isin(["standard", "frelp"])]
        .pivot_table(index="element", columns="process",
                     values="recovery_efficiency", aggfunc="first")
        .reset_index().fillna(0.0),
        ["element", "standard", "frelp"],
        ["Element", "Standard recycling", "High-value (FRELP)"],
        [str, lambda v: f"{v*100:.0f}%", lambda v: f"{v*100:.0f}%"])

    cum2050 = r.deployment.loc[r.deployment["year"] == 2050, "cumulative_gw"].iloc[0] / 1000
    crossover_txt = (f"around **{r.crossover_year}**" if r.crossover_year
                     else "after the modelled horizon")

    text = f"""# Circularity: Recycling Turns the Terawatt Ceiling Into a Moving Target
...
end-of-life projections. Horizon: {HORIZON}.*
...
- **The ceiling rises.** With high-value recycling, silicon's silver-limited
  deployment ceiling climbs from a flat **{ceil_linear:.2f} TW/yr** (mining only)
  to **{r.ceiling_2050:.2f} TW/yr by 2050** and **{r.ceiling_2080:.1f} TW/yr by
...
- **But recycling lags growth.** While deployment is still rising exponentially,
  ...
  **~{r.circ_2050_pct:.0f}%** of silver demand by 2050. Recycling does not rescue
  the *growth* phase — it secures the *steady state*. Silver crosses 50% recycled
  content {crossover_txt}, once deployment matures.
- **The urban mine is enormous.** Cumulative retired-panel mass reaches
  **~{r.mass_2050_mt:.0f} Mt by 2050** in this trajectory ({cum2050:.0f} TW
...
By 2050 the cumulative retired mass is **~{r.mass_2050_mt:.0f} Mt** — an "urban
...
By {HORIZON}, in an all-silicon counterfactual at today's silver intensity,
high-value recycling avoids mining roughly
**{r.virgin_silver_avoided_t/1000:,.0f} kilotonnes of virgin silver** — metal that
...
Silicon's silver ceiling rises past the ~{M.NET_ZERO_TW_PER_YEAR:.0f} TW/yr
...
- The deployment projection is one scenario (~{cum2050:.0f} TW by 2050); the
...
"""
    ...
    path = outdir / "REPORT_CIRCULARITY.md"
    path.write_text(text, encoding="utf-8")
    return path


def generate_circularity(outdir: Path = DEFAULT_OUTDIR) -> Path:
    results = collect_circularity_results()
    render_circularity_figures(results, outdir)
    return build_circularity_report(results, outdir)
